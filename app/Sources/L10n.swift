...
enum L10n {
    ...
    private static let pairs: [(String, String)] = [
        ...
        ("USB гостя: пакет ждал приёма %.1f с", "The guest's USB: a packet waited %.1f s to be taken"),
        ("USB гостя: пакет не ушёл — %@", "The guest's USB: a packet did not go out — %@"),
        ("USB гостя: гость молчит — повторяю последние %d пакетов", "The guest's USB: the guest has gone quiet — sending the last %d packets again"),
        ...
        ("RAM-диск не IM4P — не тот файл?", "The ramdisk is not an IM4P — the wrong file?"),
        ("В RAM-диске не найден том HFS+", "No HFS+ volume inside the ramdisk"),
        ("В RAM-диске нет файла %@", "The ramdisk has no %@"),
        ("Не влезает в %@: %d байт при месте на %d", "Does not fit in %@: %d bytes, room for %d"),
        ("В приложении нет программ для гостя", "The app carries no programs for the guest"),
        ("В RAM-диске не осталось свободных блоков", "The ramdisk has no free blocks left"),
        ("Подготовка: RAM-диск с патчером собран", "Preparing: the ramdisk carries the patcher now"),
        ("Рестор: патчер в RAM-диск не попал — %@", "Restore: the patcher did not make it into the ramdisk — %@"),
        ("Патчи файловой системы", "Filesystem patches"),
        ("Патчи: не дождались — машина останавливается как есть",
         "Patches: gave up waiting — the machine is stopped as it is"),
        ...
        ("iOS 14 не спрашивает тикет Cryptex1 — рестор без компьютера идёт до конца.",
            "iOS 14 never asks for a Cryptex1 ticket -- a restore with no computer runs all the way through."),
        ("Шаблон Cryptex1 на месте — рестор подпишет тикет сам, когда гость его попросит.",
            "The Cryptex1 template is in place -- the restore will sign the ticket itself once the guest asks for one."),
        ("iOS 16+ на середине рестора просит подписать тикет Cryptex1. Положите в набор шаблон (любой ваш собственный тикет Cryptex1) — ниже, во вкладке «С нуля», или файлом cryptex_template.im4m в готовой папке. Пока его нет, «Начать рестор» недоступен для этой версии.",
            "iOS 16+ asks partway through for a Cryptex1 ticket to be signed. Put a template in the kit -- any Cryptex1 ticket of your own -- below, in the \"From scratch\" tab, or as cryptex_template.im4m in a ready folder. \"Start the restore\" is disabled for this version until it's there."),
        ("Шаблон Cryptex1 (только iOS 16+)", "Cryptex1 template (iOS 16+ only)"),
        ("любой ваш собственный тикет Cryptex1 — форма и подпись, не содержимое",
            "any Cryptex1 ticket of your own -- its shape and signature, not what it says"),
        ("Подготовка: шаблон Cryptex1 на месте", "Prep: the Cryptex1 template is in place"),
        ("Рестор: подписан тикет Cryptex1, %d байт", "Restore: signed a Cryptex1 ticket, %d bytes"),
        ("нет шаблона Cryptex1 — см. RESTORE.md", "no Cryptex1 template -- see RESTORE.md"),
        ("Шаблон Cryptex1 устроен не так, как ожидалось: %@", "The Cryptex1 template isn't shaped the way it should be: %@"),
        ("Откуда", "Source"),
        ("С нуля", "From scratch"),
